...
do
    ...
    echo "$format, false";
    $CLICKHOUSE_CLIENT --max_threads=0 --max_block_size=65505 --input_format_skip_unknown_fields=1 --input_format_parallel_parsing=false -q "INSERT INTO parsing_with_names SELECT ClientIP6 as c, toTimeZone(ClientEventTime, 'Asia/Dubai') as a, MobilePhoneModel as b FROM test.hits LIMIT 5000 Format $format"
